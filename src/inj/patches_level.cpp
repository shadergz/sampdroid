...
{
    useriDsp("Hook: hooking member %#llx with %#llx function, and saving in %#llx", method, replace, saveIn);
    ...
    userdDsp("Hook: new trampoline allocated in %#llx address", hookCtx);
    ...
    useriDsp("Hook: installed in address [%#llx] by @%#llx@, (| %#llx | %u |)", method, replace, (uintptr_t)trContext & 0xffffffffff, hookCtx->m_iCount);
}
